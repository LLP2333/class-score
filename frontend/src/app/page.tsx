...
import { useStudentStore, useGroupStore, initDefaultData } from '@/store';
...
export default function HomePage() {
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (activeGroupId !== 'all') {
      result = result.filter(s => s.groupId === activeGroupId);
    }
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {filteredStudents.map((student) => (
            <StudentCard
              key={student.id}
              student={student}
              onAddScore={() => handleAddScore(student)}
              onMinusScore={() => handleMinusScore(student)}
