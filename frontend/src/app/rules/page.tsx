...
import { useRuleStore, useAuthStore } from '@/store';
...
export default function RulesPage() {
  ...
  const handleOpenAdd = () => {
    setEditingRule(null);
    setName(''); setScore(''); setType('add'); setCategory('学习'); setIcon('📌');
    setModalOpen(true);
  };

  const handleOpenEdit = (rule: Rule) => {
    setEditingRule(rule);
    setName(rule.name); setScore(rule.score.toString());
    setType(rule.type); setCategory(rule.category); setIcon(rule.icon);
    setModalOpen(true);
  };

  const handleSave = async () => {
    if (!name.trim()) { toast.error('请输入规则名称'); return; }
    if (!score || parseInt(score) <= 0) { toast.error('请输入有效分值'); return; }
    if (!currentClassId) return;

    if (editingRule) {
      await updateRule(editingRule.id, { name: name.trim(), score: parseInt(score), type, category, icon });
      toast.success('规则已更新');
    } else {
      await addRule(currentClassId, { name: name.trim(), score: parseInt(score), type, category, icon });
      toast.success('规则已添加');
    }
    setModalOpen(false);
  };
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-2">
            {addRules.length > 0 ? addRules.map(rule => <RuleCard key={rule.id} rule={rule} />) : <p className="text-muted-foreground text-center py-4">暂无加分规则</p>}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {minusRules.length > 0 ? minusRules.map(rule => <RuleCard key={rule.id} rule={rule} />) : <p className="text-muted-foreground text-center py-4">暂无扣分规则</p>}
          </CardContent>
        </Card>
      </div>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div><label className="text-sm font-medium mb-2 block">规则名称 *</label><Input placeholder="例如：课堂回答问题" value={name} onChange={(e) => setName(e.target.value)} /></div>
            <div><label className="text-sm font-medium mb-2 block">分值 *</label><Input type="number" min={1} value={score} onChange={(e) => setScore(e.target.value)} /></div>
            <div>
